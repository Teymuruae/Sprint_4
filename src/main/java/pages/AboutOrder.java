...
public class AboutOrder {
    ...
    //метод клика по выпадающему списку дней аренды самоката
    public void clickChosenDayInList(int daysCount, String chosenDaysCount){
        List<WebElement> orderDaysList = driver.findElements(ORDER_DAYS_COUNT);
        orderDaysList.get(daysCount).click();
        new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.textToBe(ORDER_PERION_CHOSEN_FIELD, chosenDaysCount));
    }
    ...
    public void chooseOrderDaysCount(int daysCount) {
        ...
        if (daysCount <= orderDaysList.size()) {
            ...
            clickChosenDayInList(daysCount, s1);
        } else {
            clickChosenDayInList(1, s2);
        }
    }

    public void clickCheckboxColorScooter (By color, boolean result){
        driver.findElement(color).click();
        new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.elementSelectionStateToBe(color, result));
    }

    //        метод выбора цвета самоката
    public void chooseSamokatColor(By scooterColor) {
        new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(scooterColor));

        boolean checkBox = driver.findElement(scooterColor).isSelected();
        if (checkBox) {
            clickCheckboxColorScooter(scooterColor, false);

        } else {
            clickCheckboxColorScooter(scooterColor, true);
